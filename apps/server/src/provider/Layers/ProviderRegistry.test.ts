...
// These tests exercise snapshot persistence and merging with made-up model
// names, so they run with an unrestricted model list. The server-owned list is
// proven in packages/contracts/src/modelOffering.test.ts and in the
// "applies the server-owned model list" test below, which uses the real one.
const UNRESTRICTED_OFFERING_SETTINGS: ContractServerSettings = {
  ...DEFAULT_SERVER_SETTINGS,
  modelOffering: { offeredModels: {}, retiredModels: [] },
};
...
it.layer(
  Layer.mergeAll(
    ...
    Layer.effect(
      ...
      makeMutableServerSettingsService(UNRESTRICTED_OFFERING_SETTINGS),
    ),
    TestHttpClientLive,
  ),
)("ProviderRegistry", (it) => {
  ...
  describe("ProviderRegistryLive", () => {
    ...
    it.effect("applies the server-owned model list to every provider list it hands out", () =>
      Effect.gen(function* () {
        const codexDriver = ProviderDriverKind.make("codex");
        const codexInstanceId = ProviderInstanceId.make("codex");
        ...
        const provider = {
          ...
          version: "0.0.0-test",
          models: [model("gpt-5.5"), model("vega-alpha"), model("gpt-6-astra")],
          slashCommands: [],
          skills: [],
        } as const satisfies ServerProvider;
        ...
        // Ryan's compiled list, not the unrestricted one the persistence tests use.
        const settings = yield* makeMutableServerSettingsService(DEFAULT_SERVER_SETTINGS);
        const scope = yield* Scope.make();
        yield* Effect.addFinalizer(() => Scope.close(scope, Exit.void));
        const runtimeServices = yield* Layer.build(
          ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "t3-provider-registry-model-offering-",
              }),
            ),
        ...
        yield* Effect.gen(function* () {
          const registry = yield* ProviderRegistry.ProviderRegistry;
          const slugs = (providers: ReadonlyArray<ServerProvider>) =>
            providers[0]?.models.map((entry) => entry.slug);

          // Retired and unlisted ids are absent; the allowed id is present.
          assert.deepStrictEqual(slugs(yield* registry.getProviders), ["gpt-6-astra"]);
          assert.deepStrictEqual(slugs(yield* registry.refreshInstance(codexInstanceId)), [
            "gpt-6-astra",
          ]);

          // A written list replaces the compiled one, whole, without a restart.
          yield* settings.updateSettings({
            modelOffering: { offeredModels: { codex: ["vega-alpha"] }, retiredModels: [] },
          });
          assert.deepStrictEqual(slugs(yield* registry.getProviders), ["vega-alpha"]);
        }).pipe(Effect.provide(runtimeServices));
      }),
    );
